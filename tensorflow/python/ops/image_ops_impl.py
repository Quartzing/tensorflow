...
def non_max_suppression_soft(boxes,
                              scores,
                              max_output_size,
                              iou_threshold=0.0,
                              score_threshold=0.0,
  ...
  using the `tf.gather` operation.  For example:
    ```python
    selected_indices = tf.image.non_max_suppression_soft(
        boxes, scores, max_output_size, iou_threshold, 'linear')
  ...
  Args:
    ...
    method: choose between 'linear' and 'gaussian'
    name: A name for the operation (optional).
  ...
  with ops.name_scope(name, 'non_max_suppression'):
    if method == 'linear':
        ...
        if sigma <= 0:
            raise ValueError("sigma should be positive in Gaussian mode. Sigma given: {}".format(sigma))
    iou_threshold = ops.convert_to_tensor(iou_threshold, name='iou_threshold')
    score_threshold = ops.convert_to_tensor(
        score_threshold, name='score_threshold')
    
    return gen_image_ops.non_max_suppression_soft(boxes, scores, max_output_size,
