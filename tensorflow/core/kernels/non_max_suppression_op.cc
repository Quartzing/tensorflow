...
template <typename T>
static inline T IOUBetweenBoxes(typename TTypes<T, 2>::ConstTensor boxes, int i,
                                int j) {
...
template <typename T>
T AdjustScoreInSoftNMS(T iou, const float iou_threshold = 0,
                       const float sigma = 0) {
  if (sigma <= 0) {
    if (iou < static_cast<T>(iou_threshold)) {
      return static_cast<T>(1.0);
    } else {
      return static_cast<T>(1.0) - iou;
    }
  } else {
...
template <typename T>
void DoNonMaxSuppressionSoftOp(
    OpKernelContext* context, const Tensor& scores, const Tensor& boxes,
    int num_boxes, const Tensor& max_output_size, const float score_threshold,
    const float iou_threshold = 0, const float sigma = 0,
    bool pad_to_max_output_size = false, int* ptr_num_valid_outputs = nullptr) {
  OP_REQUIRES(context, iou_threshold >= 0 && iou_threshold <= 1,
              errors::InvalidArgument("iou_threshold must be within [0, 1]"));
  OP_REQUIRES(context, sigma >= 0,
              errors::InvalidArgument("sigma must be >= 0"));
  typename TTypes<T, 2>::ConstTensor boxes_data = boxes.tensor<T, 2>();
  ...
  while (selected.size() < output_size && !candidate_priority_queue.empty()) {
    ...
    for (int j = static_cast<int>(selected.size()) - 1; j >= 0; --j) {
      iou = IOUBetweenBoxes<T>(boxes_data, next_candidate.box_index, selected[j]);
      weight = AdjustScoreInSoftNMS<T>(iou, iou_threshold, sigma);
      if (static_cast<float>(next_candidate.score * weight) < score_threshold) {
        should_select = false;
        break;
      }
    }
...
template <typename Device, typename T>
class NonMaxSuppressionSoftOp : public OpKernel {
 public:
  explicit NonMaxSuppressionSoftOp(OpKernelConstruction* context)
      : OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(
        context, TensorShapeUtils::IsScalar(sigma.shape()),
        errors::InvalidArgument("sigma must be 0-D, got shape ",
                                sigma.shape().DebugString()));
    const float sigma_val = sigma.scalar<float>()();
    ...
    DoNonMaxSuppressionSoftOp<T>(context, scores, boxes, num_boxes,
                                 max_output_size, score_threshold_val,
                                 iou_threshold_val, sigma_val);
  }
};
...
REGISTER_KERNEL_BUILDER(
    Name("NonMaxSuppressionSoft").TypeConstraint<float>("T").Device(DEVICE_CPU),
    NonMaxSuppressionSoftOp<CPUDevice, float>);
REGISTER_KERNEL_BUILDER(Name("NonMaxSuppressionSoft")
                            .TypeConstraint<Eigen::half>("T")
                            .Device(DEVICE_CPU),
                        NonMaxSuppressionSoftOp<CPUDevice, Eigen::half>);
